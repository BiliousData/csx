...
#include "stage/alleya.h"
#include "stage/alleyb.h"
...
static void Stage_DrawHealth(s16 health, u8 i, s8 ox)
{
	...
	s8 dying;
	if (ox < 0)
		dying = (health >= 18000) * 48;
	else
		dying = (health <= 2000) * 48;
	...
	RECT_FIXED dst = {
		hx + ox * FIXED_DEC(20,1) - FIXED_DEC(22,1),
		FIXED_DEC(SCREEN_HEIGHT2 - 47  + 4 - 12, 1),
		src.w << FIXED_SHIFT,
		src.h << FIXED_SHIFT
	};
	if (stage.downscroll)
