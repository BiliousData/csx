...
static struct
{
	...
	//Menu assets
	Gfx_Tex tex_back, tex_main, tex_ng, tex_story, tex_title;
	FontData font_bold, font_arial;
	
	Character *gf; //Title Girlfriend
	Character *kass; //Menu Kassetto W.I.P
} menu;
...
static void Menu_DrawMain(boolean flash, s32 scroll, u8 r0, u8 g0, u8 b0, u8 r1, u8 g1, u8 b1)
{
	RECT main_src = {0, 0, 255, 255};
	RECT main_dst = {0, -scroll - SCREEN_WIDEADD2, SCREEN_WIDTH, SCREEN_WIDTH * 4 / 5};
	
	if (flash || (animf_count & 4) == 0)
		Gfx_DrawTexCol(&menu.tex_main, &main_src, &main_dst, r0, g0, b0);
	else
		Gfx_DrawTexCol(&menu.tex_main, &main_src, &main_dst, r1, g1, b1);
}
...
void Menu_Load(MenuPage page)
{
	...
	menu.kass = Char_CGM_New(FIXED_DEC(-77,1), FIXED_DEC(76,1));
...
void Menu_Tick(void)
{
	...
	//Get song position
	u16 next_step = Audio_TellXA_Milli() / 165; //100 BPM
	...
	//Fallthrough
		...
		case MenuPage_Main:
		{
			...
			if (menu.next_page == menu.page && Trans_Idle())
			{
				...
				if (pad_state.press & (PAD_DOWN | PAD_UP)) //Menu animation stuff
				{
					switch (menu.select) //plays different animation depending on option selected
					{
						case 0:
						menu.kass->set_anim(menu.kass, CharAnim_Idle); //Play
						break;
						case 1:
						menu.kass->set_anim(menu.kass, CharAnim_Left); //Freeplay
						break;
						case 2:
						menu.kass->set_anim(menu.kass, CharAnim_LeftAlt); //Options
						break;
						case 3:
						menu.kass->set_anim(menu.kass, CharAnim_Idle);//Credits
						break;
					}
				}
			...
			s32 next_scroll = menu.select *
				...
				FIXED_DEC(0,1);
			#endif
			menu.scroll += (next_scroll - menu.scroll) >> 0;
		...
		case MenuPage_Story:
		{
			static const struct
			{
				...
				{"1", StageId_1_1, "DADDY DEAREST", {"BIG", "BIG", "CHungus"}},
			};
		...
		case MenuPage_Credits:
		{
			static const struct
			{
				...
				{StageId_1_1, "CASSETTE GIRL PORT", false},
				{StageId_1_1, "", false},
				{StageId_1_1, "BILIOUSDATA", false},
				{StageId_1_1, "CODING", false},
				{StageId_1_1, "MENU SPRITES", false},
				{StageId_1_1, "BACKGROUNDS", false},
				{StageId_1_1, "", false},
				{StageId_1_1, "UNSTOPABLE", false},
				{StageId_1_1, "INGAME CG SPRITES", false},
			};
		...
		case MenuPage_Options:
		{
			...
			static const struct
			{
				...
				//{OptType_Stage, "SUPER SECRET SONG", &stage.nothing, {.spec_boolean = {0}}},
			};
			...
			for (u8 i = 0; i < COUNT_OF(menu_options); i++)
			{
				...
				switch (menu_options[i].type)
				{
					...
					case OptType_Enum:
						...
						break;
					case OptType_Stage:
					    sprintf(text, "%s %s", menu_options[i].text, *((boolean*)menu_options[i].value) ? "" : "");
						break;
				}
				menu.font_bold.draw(&menu.font_bold,
