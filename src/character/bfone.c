...
static SkullFragment char_bfone_skull[15] = {
	{ 1 * 8, -87 * 8, -13, -13},
	{ 9 * 8, -88 * 8,   5, -22},
	{18 * 8, -87 * 8,   9, -22},
	{26 * 8, -85 * 8,  13, -13},
...
typedef struct
{
	...
	SkullFragment skull[COUNT_OF(char_bfone_skull)];
	u8 skull_scale;
} Char_BFONE;

//Boyfriend player definitions
static const CharFrame char_bfone_frame[] = {
...
static const Animation char_bfone_anim[PlayerAnim_Max] = {
...
void Char_BFONE_SetFrame(void *user, u8 frame)
{
	Char_BFONE *this = (Char_BFONE*)user;
	
	//Check if this is a new frame
	if (frame != this->frame)
	{
		//Check if new art shall be loaded
		const CharFrame *cframe = &char_bfone_frame[this->frame = frame];
		if (cframe->tex != this->tex_id)
			Gfx_LoadTex(&this->tex, this->arc_ptr[this->tex_id = cframe->tex], 0);
	}
}

void Char_BFONE_Tick(Character *character)
{
	Char_BFONE *this = (Char_BFONE*)character;
	...
	if (character->animatable.anim >= PlayerAnim_Dead3)
	{
		...
		if (this->skull_scale)
		{
			...
			for (size_t i = 0; i < COUNT_OF_MEMBER(Char_BFONE, skull); i++, frag++)
			{
				//Draw fragment
				RECT frag_src = {
					(i & 1) ? 112 : 96,
	...
	Animatable_Animate(&character->animatable, (void*)this, Char_BFONE_SetFrame);
	Character_Draw(character, &this->tex, &char_bfone_frame[this->frame]);
}

void Char_BFONE_SetAnim(Character *character, u8 anim)
{
	Char_BFONE *this = (Char_BFONE*)character;
...
void Char_BFONE_Free(Character *character)
{
	Char_BFONE *this = (Char_BFONE*)character;
...
Character *Char_BFONE_New(fixed_t x, fixed_t y)
{
	//Allocate boyfriend object
	Char_BFONE *this = Mem_Alloc(sizeof(Char_BFONE));
	if (this == NULL)
	{
		sprintf(error_msg, "[Char_BFONE_New] Failed to allocate boyfriend object");
		ErrorLock();
		return NULL;
	}
	
	//Initialize character
	this->character.tick = Char_BFONE_Tick;
	this->character.set_anim = Char_BFONE_SetAnim;
	this->character.free = Char_BFONE_Free;
	
	Animatable_Init(&this->character.animatable, char_bfone_anim);
	...
	memcpy(this->skull, char_bfone_skull, sizeof(char_bfone_skull));
	this->skull_scale = 64;
